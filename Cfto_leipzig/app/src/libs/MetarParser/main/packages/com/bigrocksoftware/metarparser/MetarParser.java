...
public class MetarParser {
	...
	private ArrayList metarParts = new ArrayList();
	...
	private Metar parseData(String metarData) throws MetarParseException {
		...
		log.debug("MetarParser: raw: "+metarData);

		Metar metar = new Metar();

		// test data
		//metarData = "2003.05.05 2153 UTC\n";
		...
		try {
			utility.split(metarParts, "/\\n/", metarData);
		} catch(MalformedPerl5PatternException e) {
			log.error("MetarParser: error spliting metar data on whitespace: "+e);
			throw new MetarParseException("error spliting metar data on whitespace: "+e);
		}

		// grab the first line of the report, which is a date string
		try {
			dateString = (String)metarParts.get(0);
			metar.setDateString(dateString);
			log.debug("MetarParser: found date string '"+((String)metarParts.get(0))+"'");
		} catch(ArrayIndexOutOfBoundsException e) {
			log.error("MetarParser: no data found in metar string: "+e);
			throw new MetarParseException("no data found in metar string: "+e);
		}
			
		// split the second line, the METAR data, on whitespace into tokens for
		// processing
		try {
			utility.split(tokens, ((String)metarParts.get(1)));
		} catch(MalformedPerl5PatternException e) {
			log.error("MetarParser: error spliting metar data on whitespace: "+e);
			throw new MetarParseException("error spliting metar data on whitespace: "+e);
		}
		...
		if (((String)tokens.get(index)).endsWith("Z")) {
			...
			calendar.setTime(sdf.parse(dateString, new ParsePosition(0)));

			log.debug("MetarParser: date: using year='"+calendar.get(Calendar.YEAR)+"'");
			log.debug("MetarParser: date: using month='"+calendar.get(Calendar.MONTH)+"'");

			int dayInt, hourInt, minuteInt;
			try {
				String day = ((String)tokens.get(index)).substring(0,2);
				log.debug("MetarParser: date: got '"+day+"' for day");

				String hour = ((String)tokens.get(index)).substring(2,4);
				log.debug("MetarParser: date: got '"+hour+"' for hour");

				String minute = ((String)tokens.get(index)).substring(4,6);
				log.debug("MetarParser: date: got '"+minute+"' for minute");
				...
				log.error("MetarParser: unable to parse Metar date value: "+nfe);
		    ...
		    ((String)tokens.get(index)).equals(MetarConstants.METAR_CORRECTED))
		{
			...
			log.debug("MetarParser: report modifier: "+metar.getReportModifier());
		} else {
		...
		if (temp.endsWith("KT") || temp.endsWith("MPS")) {
			...
			try {
				...
				log.error("MetarParser: error determining 3 digit wind speed: "+e);
			}
			...
			if (((String)tokens.get(index)).charAt(pos) == 'G') {
				...
				try {
					...
					log.error("MetarParser: error matching wind gusts: "+e);
				}
			} else {
				// we don't have gusts
			...
			log.debug("MetarParser: wind direction: "+metar.getWindDirection());
			if (windInKnots) {
				log.debug("MetarParser: wind speed: "+metar.getWindSpeedInKnots());
			} else {
				log.debug("MetarParser: wind speed: "+metar.getWindSpeedInMPS());
			}
			log.debug("MetarParser: wind gusts: "+metar.getWindGustsInKnots());
			...
			try {
				if (matcher.matches(temp, new Perl5Compiler().compile(".*\\d\\d\\dV\\d\\d\\d")))
				{
					...
					log.debug("MetarParser: variable wind direction min: " + metar.getWindDirectionMin());
					log.debug("MetarParser: variable wind direction max: " + metar.getWindDirectionMax());
				...
				log.error("MetarParser: error matching variable wind speed: "+e);
			}
		} else {
		...
		try {
					   ...
					   ((index+1 < numTokens) && ((String)tokens.get(index+1)).endsWith("KM")))
			{
				...
				if (!fraction.equals("")) {
					...
					try {
						...
						log.error("MetarParser: error spliting fraction on /: "+e);
				...
				log.debug("MetarParser: visibility: "+metar.getVisibility()+" M");
			} else {
				String token = (String)tokens.get(index);
				boolean isLessThan = false;
			...
			log.error("MetarParser: error matching visibilities in meters: "+e);
		}
		...
		while (((String)tokens.get(index)).startsWith("R")) {
			...
			log.debug("MetarParser: RVR runway number: "+new Integer(((String)tokens.get(index)).substring(1,3)));

			int pos = 3;
			if (((String)tokens.get(index)).charAt(pos) != '/') {
				runwayVisualRange.setApproachDirection(((String)tokens.get(index)).charAt(pos));
				log.debug("MetarParser: RVR runway approach direction: "+((String)tokens.get(index)).charAt(pos));
			...
			switch (((String)tokens.get(index)).charAt(pos)) {
				...
				case 'M': // above 6000ft
					...
					log.debug("MetarParser: RVR modifier: "+((String)tokens.get(index)).charAt(pos));
					pos++;
			}
			runwayVisualRange.setLowestReportable(new Integer(((String)tokens.get(index)).substring(pos,pos+4)).intValue());
			log.debug("MetarParser: RVR lowest reportable: "+new Integer(((String)tokens.get(index)).substring(pos,pos+4)));
			pos += 4;
			// if we are using the format with highest reportable
			if (((String)tokens.get(index)).length() > pos && ((String)tokens.get(index)).charAt(pos) == 'V') {
				pos++; // increment past V
				if (((String)tokens.get(index)).charAt(pos) == 'P') pos++;
				runwayVisualRange.setHighestReportable(new Integer(((String)tokens.get(index)).substring(pos,pos+4)).intValue());
				log.debug("MetarParser: RVR highest reportable: "+new Integer(((String)tokens.get(index)).substring(pos,pos+4)));
			}
		       ...
		       ((String)tokens.get(index)).startsWith(MetarConstants.METAR_NO_SIGNIFICANT_CHANGE))
		{
			    ...
			    ((String)tokens.get(index)).startsWith(MetarConstants.METAR_LIGHT))
			{
				...
				log.debug("MetarParser: weather group: intensity: "+((String)tokens.get(index)).charAt(0));
				pos++;
			} else {
				log.debug("MetarParser: weather group: intensity: moderate");
			}
		        ...
		        ((String)tokens.get(index)).substring(pos,pos+2).startsWith(MetarConstants.METAR_FREEZING))
			{
				...
				log.debug("MetarParser: weather group: descriptor: "+((String)tokens.get(index)).substring(pos,pos+2));
				pos += 2;
			} else {
				log.debug("MetarParser: weather group: descriptor: no descriptor");
			}

			// if we have phenomena
		    if (((String)tokens.get(index)).length()>pos+2 && (((String)tokens.get(index)).substring(pos,pos+2).equals(MetarConstants.METAR_DRIZZLE) ||
		        ...
		        ((String)tokens.get(index)).substring(pos,pos+2).equals(MetarConstants.METAR_DUST_STORM)))
			{
				...
				log.debug("MetarParser: weather group: phenomena: "+((String)tokens.get(index)).substring(pos,pos+2));
				metar.addWeatherCondition(weatherCondition);
				log.debug("MetarParser: "+weatherCondition.getNaturalLanguageString());
			} else {
		       ...
		       ((String)tokens.get(index)).startsWith(MetarConstants.METAR_NO_SIGNIFICANT_CLOUDS))
		{
		        ...
		        ((String)tokens.get(index)).startsWith(MetarConstants.METAR_OVERCAST))
			{
				...
				log.debug("MetarParser: sky condition: contraction: "+((String)tokens.get(index)).substring(0,3));
				skyCondition.setHeight(new Integer(((String)tokens.get(index)).substring(3,6)).intValue());
				log.debug("MetarParser: sky condition: height: "+((String)tokens.get(index)).substring(3,6));
				if (((String)tokens.get(index)).length() > 6) {
					...
					log.debug("MetarParser: sky condition: modifier: "+((String)tokens.get(index)).substring(6,((String)tokens.get(index)).length()));
				}
			} else if (((String)tokens.get(index)).startsWith(MetarConstants.METAR_SKY_CLEAR) ||
		               ((String)tokens.get(index)).startsWith(MetarConstants.METAR_CLEAR))
			{
				skyCondition.setContraction(((String)tokens.get(index)).substring(0,3));
				log.debug("MetarParser: sky condition: clear: " + ((String)tokens.get(index)).substring(0,3));
				...
				log.debug("MetarParser: sky condition: contraction: " + ((String)tokens.get(index)).substring(0,2));
				try{
    				skyCondition.setHeight(new Integer(((String)tokens.get(index)).substring(2,5)).intValue());
    				log.debug("MetarParser: sky condition: height: " + ((String)tokens.get(index)).substring(2,5));
				}
				catch(NumberFormatException e){
				    log.debug("Invalid sky condition, contraction height");
				}
			} else if (((String)tokens.get(index)).startsWith(MetarConstants.METAR_NO_SIGNIFICANT_CLOUDS)) {
				skyCondition.setContraction(((String)tokens.get(index)).substring(0,3));
				log.debug("MetarParser: sky condition: No Significant Clouds: " + ((String)tokens.get(index)).substring(0,3));
			} else {
				log.debug("MetarParser: sky condition: couldn't determine which");
			}

			metar.addSkyCondition(skyCondition);
			log.debug("MetarParser: "+skyCondition.getNaturalLanguageString());
		...
		if (((String)tokens.get(index)).indexOf("/") != -1) {
			...
			log.debug("MetarParser: temperature: "+temperature+" C, " + (temperature.floatValue()*9/5+32) + " F");
			...
			log.debug("MetarParser: dew point: "+dewPoint+" C, " + (dewPoint.floatValue()*9/5+32) + " F");
		...
		if (((String)tokens.get(index)).startsWith("A")) {
			...
			log.debug("MetarParser: pressure: "+metar.getPressure()+" Hg");
		...
		while (index < numTokens) {
			...
			try {
				if (matcher.matches(temp, new Perl5Compiler().compile("T\\d{8}"))) {
					log.debug("MetarParser: found detailed temp: "+((String)tokens.get(index)));
					// we have a sub-zero temperature
					...
					log.debug("MetarParser: temperature precise: "+temperaturePrecise+" C, " + (temperaturePrecise.floatValue()*9/5+32) + " F");
					log.debug("MetarParser: dew point precise: "+dewPointPrecise+" C, " + (dewPointPrecise.floatValue()*9/5+32) + " F");
						   ...
						   ((String)tokens.get(index)).equals(MetarConstants.METAR_SPRAY))
				{
					...
					log.debug("MetarParser: weather group: phenomena: "+((String)tokens.get(index)));
						...
						((String)tokens.get(index)).startsWith(MetarConstants.METAR_OVERCAST))
					{
						...
						log.debug("MetarParser: "+obscuration.getNaturalLanguageString());
					}
				...
				log.error("MetarParser: error matching additional remarks: "+e);
			}
